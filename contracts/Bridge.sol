...
contract Bridge {
    ...
    modifier issuerMustBePending(string calldata issuer) {
        require(issuers[issuer].status == Status.PENDING, "The issuer is not in the PENDING state.");
        _;
    }

    modifier issuerDoesNotExist(string calldata issuer) {
        require(issuers[issuer].sender == address(0), "An issuer already exists with this address.");
        _;
    }
     ...
     * @return issuer list
     */
    function getIssuers() external view returns (string[] memory) {
        return issuerList;
    }
     ...
     */
    function getVerifiedIssuers() external view returns (string[] memory) {
     ...
     * @return status
     */
    function getIssuerStatus(string calldata issuer) external view returns (Status status) {
     ...
     **/
    function createIssuer(string calldata issuer, uint256 amount) external issuerDoesNotExist(issuer) {
     ...
     **/
    function cancelIssuer(string calldata issuer) external issuerExists(issuer) issuerMustBePending(issuer) {
        require(issuers[issuer].sender == msg.sender, "Only the originating account can cancel this issuer.");
    ...
    function completeIssuance(
        ...
        verifyPaymentFinality(txHash, source, issuer, destinationTag, amount);
    ...
    function proveFraud(
        ...
        require(issuers[issuer].xrplTxId != txHash, "The provided transaction has already been proved.");
     ...
     * @param issuer the issuer address of the tx
     **/
    function createRedemptionReservation(string calldata source, string calldata issuer) external {
    ...
    function createRedemptionReservationHash(string calldata source, string calldata issuer)
        public
        pure
        returns (bytes32 redepmtionHash)
    {
     ...
     * @param issuer the issuer address of the tx
     **/
    function cancelRedemptionReservation(string calldata source, string calldata issuer) external {
        bytes32 redemptionHash = createRedemptionReservationHash(source, issuer);
        require(reservations[redemptionHash].redeemer == msg.sender, "There is no reservation for this issuer.");
    ...
    function completeRedemption(
        ...
        require(destAddress != address(0), "Destination address cannot be the zero address.");
        ...
        verifyPaymentFinality(txID, source, issuer, 0, amount);
        issuers[issuer].amount = issuers[issuer].amount - amount;
        unlockAsset(destAddress, amount);
        redemptions[txID] = Redemption(source, issuer, amount, destAddress, msg.sender);
